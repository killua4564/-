...
class Team:
    ...
    def center_key(self, center, member):
        if center.name.split('_')[0] == 'UR':
            key1 = 0.09
            if center.center_skill == 'Team' and center.team == member.team: key2 = 0.03 
            elif center.center_skill == 'Grade' and center.grade == member.grade: key2 = 0.06
            elif center.center_skill == 'Unit' and center.unit == member.unit: key2 = 0.06
            else: key2 = 0.0
        elif center.name.split('_')[0] == 'SSR':
            key1 = 0.07
            if center.center_skill == 'Team' and center.team == member.team: key2 = 0.01
            elif center.center_skill == 'Grade' and center.grade == member.grade: key2 = 0.02
            elif center.center_skill == 'Unit' and center.unit == member.unit: key2 = 0.02
            else: key2 = 0.0
        else: key1, key2 = 0.0, 0.0
        return key1, key2
                
    def perfect_fill(self):
        self.perfect_rate = float(open('./perfect_rate', 'r').read())
        self.score_to_point = 1 / (0.015972 + 0.002178 * self.perfect_rate)
        for member in self.members:
            if member.hole == 2:
                #2 personal 10%
                personal = self.personal(member, 0.1)
                ...
                personal = self.personal(member, 0.16)
                team = self.team(0.018)
                self.avg_score += member.add_score * self.score_to_point
                if personal > team:
                    member.added += personal
                    self.score += personal
                ...
                personal = self.personal(member, 0.16) + 200
                team = self.team(0.024)
                ...
                #5 personal 26%, team 1.8% + personal 10%, team 2.4% + 200, add_score x2.5 + 200
                personal = self.personal(member, 0.26)
                team_personal = self.team(0.018) + self.personal(member, 0.1)
                team = self.team(0.024) + 200
                add_score = member.add_score * 2.5 * self.score_to_point + 200
                maximum = max(personal, team_personal, team, add_score)
                if personal == maximum:
                    member.added += personal
                    self.score += personal
                    self.avg_score += member.add_score * self.score_to_point
                    ...
                    self.team_add(0.018)
                    member.added += self.personal(member, 0.1)
                ...
                #6 personal 16% + team 1.8%, team 2.4% + personal 10%, add_score x2.5 + personal 10%
                personal_team = self.personal(member, 0.16) + self.team(0.018)
                team_personal = self.team(0.024) + self.personal(member, 0.1)
                add_score = member.add_score * 2.5 * self.score_to_point + self.personal(member, 0.1)
                maximum = max(personal_team, team_personal, add_score)
                if maximum == personal_team:
                    member.added += self.personal(member, 0.16)
                    self.team_add(0.018)
                    ...
                    member += self.personal(member, 0.1)
                    self.score += self.personal(member, 0.1)
                    self.avg_score += member.add_score * 2.5 * self.score_to_point
